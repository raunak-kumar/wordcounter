...
function App() {
  ...
  let randomText = useMemo(() => rawData[Math.floor(Math.random() * rawData.length)],[])

  const handleInput = (e) => {
    e.preventDefault();
    let inputData = e.target.value
    if (inputData[0] === randomText[0] && !toggleTimer) {
      setToggleTimer(true)
    }

    let randorArr = randomText.split(" ");
    let inputArr = inputData.split(" ");
    let correct = 0
    let incorrect = 0
    for (let i = 0; i < inputArr.length; i++) {
      if (inputArr[i] === randorArr[i]) {
        correct++
      }
      else if(inputArr[i] !== randorArr[i] && inputArr[i] !== ""){
        incorrect++
      }
    }
   setWordDetails({
  ...
  return (
    <div className="App">
      ...
      <div><textarea disabled defaultValue={randomText} /></div>
      <div><textarea onChange={e => handleInput(e)} /></div>
      <div><button onClick={() => window.location.reload()}>Reset</button></div>
      {timeZero === true && <Details time={timeZero} wordDetails={wordDetails}></Details>}
    </div>
  );
}
